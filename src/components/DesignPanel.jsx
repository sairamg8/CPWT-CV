    ...
    <div className="space-y-6 py-2">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-[10px] text-amber-600 mt-0.5">
              {confirmReset
                ? 'This will reset all design settings (font, colors, spacing, etc.) to ATS defaults. Your resume content is kept.'
                : 'Resets font, colors, spacing, and layout settings to ATS-safe defaults. Resume content and personal info are preserved.'}
            </p>
          </div>
          {confirmReset ? (
            ...
            <button
              ...
              className="px-3 py-1.5 text-xs font-semibold text-amber-700 bg-amber-100 hover:bg-amber-200 border border-amber-300 rounded-lg transition-colors shrink-0"
            >
              Reset
            </button>
          )}
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {TEMPLATES.map(t => (
            ...
            >
              <div
                className={`w-8 h-10 rounded shrink-0 flex flex-col gap-0.5 p-1 ${
                  resume.template === t.id ? 'opacity-100' : 'opacity-40'
                }`}
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  <p className={`text-sm font-medium ${resume.template === t.id ? 'text-blue-700' : 'text-gray-700'}`}>
                    {t.label}
                  </p>
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={() => updateSetting('sectionBorderWidth', Math.max(1, (settings.sectionBorderWidth ?? 1) - 1))}
              className="w-6 h-6 flex items-center justify-center border border-gray-200 rounded text-gray-600 hover:bg-gray-100 text-base leading-none"
            >−</button>
            <input
              type="number"
              min={1} max={8}
              value={settings.sectionBorderWidth ?? 1}
              onChange={e => {
                const v = parseInt(e.target.value, 10);
                if (!isNaN(v)) updateSetting('sectionBorderWidth', Math.min(8, Math.max(1, v)));
              }}
              className="w-10 text-center text-xs font-medium text-gray-700 border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-400 h-6"
            />
            <button
              onClick={() => updateSetting('sectionBorderWidth', Math.min(8, (settings.sectionBorderWidth ?? 1) + 1))}
              className="w-6 h-6 flex items-center justify-center border border-gray-200 rounded text-gray-600 hover:bg-gray-100 text-base leading-none"
            >+</button>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-gray-400 font-mono">
              {settings.sectionBorderColor || 'accent'}
            </span>
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {[
            ...
            return (
              <button
                key={opt.value}
                onClick={() => updateSetting('headingStyle', opt.value)}
              ...
              >
                <div className="mb-1">
                  {opt.value === 'ruled' && (
                    <div>
                      <span className="text-[8px] font-bold uppercase tracking-wider" style={{ color: '#374151' }}>ABC</span>
                      <div className="h-px mt-0.5" style={{ backgroundColor: '#e5e7eb' }} />
                    </div>
                  )}
                  {opt.value === 'leftbar' && (
                    <div className="flex items-center gap-1">
                      <div className="w-0.5 self-stretch rounded-full" style={{ backgroundColor: accent }} />
                      <span className="text-[8px] font-bold uppercase tracking-wider" style={{ color: '#374151' }}>ABC</span>
                    </div>
                  )}
                  {opt.value === 'line' && (
                    <div className="flex items-center gap-1">
                      <span className="text-[8px] font-bold uppercase tracking-wider" style={{ color: accent }}>ABC</span>
                      <span className="flex-1 h-px" style={{ backgroundColor: accent + '60' }} />
                    </div>
                  )}
                  {opt.value === 'underline' && (
                    <div className="pb-0.5 inline-block" style={{ borderBottom: `1.5px solid ${accent}` }}>
                      <span className="text-[8px] font-bold uppercase tracking-wider" style={{ color: accent }}>ABC</span>
                    </div>
                  )}
                  {opt.value === 'box' && (
                    <span className="text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded" style={{ color: accent, backgroundColor: accent + '18' }}>ABC</span>
                  )}
                  {opt.value === 'plain' && (
                    <span className="text-[8px] font-bold uppercase tracking-wider" style={{ color: accent }}>ABC</span>
                  )}
                  ...
                  >
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (settings.customFont === name) updateSetting('customFont', '');
                      }}
      ...
      <div>
        ...
        <div className="space-y-2.5 p-3 bg-gray-50 rounded-lg border border-gray-100">
          <NumberRow
            label="Top / Bottom"
            value={settings.marginV ?? 14}
            onChange={v => updateSetting('marginV', v)}
            min={0} max={40} step={1} unit="mm"
          />
          <NumberRow
            label="Left / Right"
            value={settings.marginH ?? 18}
            onChange={v => updateSetting('marginH', v)}
            min={0} max={40} step={1} unit="mm"
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-2.5 p-3 bg-gray-50 rounded-lg border border-gray-100">
          <NumberRow
            ...
            value={settings.sectionGap ?? 16}
            onChange={v => updateSetting('sectionGap', v)}
            min={0} max={60} step={1} unit="px"
          />
          <NumberRow
            label="Between Items"
            value={settings.itemGap ?? 12}
            onChange={v => updateSetting('itemGap', v)}
            ...
            return (
              <>
                <SizeRow
                  label="Base"
                  value={base}
                  onChange={v => updateSetting('fontSizeBase', v)}
                  min={8} max={16}
                />
                <SizeRow
                  label="Full Name"
                  value={base + nameDelta}
                  onChange={v => updateSetting('fontSizeNameDelta', v - base)}
                  min={base} max={36}
                />
                <SizeRow
                  label="Section Title"
                  value={base + sectionDelta}
                  onChange={v => updateSetting('fontSizeSectionDelta', v - base)}
                  min={6} max={24}
                />
                <SizeRow
                  label="Entry Header"
                  value={base + entryDelta}
                  onChange={v => updateSetting('fontSizeEntryDelta', v - base)}
                  min={6} max={24}
                />
                <SizeRow
                  label="Contact Icons"
                  value={settings.iconSize ?? 11}
